...
def register_as(name, password)
  ...
  @browser.find_element(id: 'user_firstname').send_keys name + 'John'
  @browser.find_element(id: 'user_lastname').send_keys 'Smith'
  @browser.find_element(id: 'user_mail').send_keys name + '@exmpl.com'
...
def displaying_bug_on_issues_page(bug_name)
  fail 'Test is failed' unless @browser.find_element(link: bug_name).displayed?
end

def displaying_feature(feature_name)
  fail 'Test is failed' unless @browser.find_element(link: feature_name).displayed?
end

def displaying_support_ticket(support_ticket_name)
  fail 'Test is failed' unless @browser.find_element(link: support_ticket_name).displayed?
end

open_register_page
register_as('bull1', '112233')
sign_out
register_as('bull2', '112233')
sign_out
sing_in_as('troll48', '223344')
